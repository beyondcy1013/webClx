...
def inspect_settings(browser, viewport, screenshot_path):
    ...
    control = page.locator("#compile-max-concurrency-input")
    control.wait_for(state="visible", timeout=10_000)

    assert control.input_value() == "5"
    assert control.get_attribute("min") == "1"
    assert control.get_attribute("max") == "32"
    assert control.get_attribute("step") == "1"
    assert page.locator("#settings-panel-system").is_visible()

    geometry = page.evaluate(
        """
        () => {
          const control = document.querySelector('#compile-max-concurrency-input');
    ...
    with sync_playwright() as playwright:
        ...
        result = {
            "desktop": inspect_settings(
                ...
                "/tmp/webclx-compile-concurrency-desktop.png",
            ),
            "mobile": inspect_settings(
                browser,
                {"width": 390, "height": 844},
                "/tmp/webclx-compile-concurrency-mobile.png",
            ),
        }
        browser.close()
