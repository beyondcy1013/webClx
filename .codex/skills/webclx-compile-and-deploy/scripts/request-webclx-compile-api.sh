#!/usr/bin/env bash
# Queue a pure project compile through the running webClx compile API.
set -euo pipefail
...
NOTE="当前代理请求通过 /api/build/compile 编译当前项目。"
DEBOUNCE_SECS=""
SKIP_SYNC=false
SKIP_RESTART=false
COMMAND_JSON=""
...
normalize_note_for_submit() {
  local note="$1"
  case "$note" in
    "Codex requested project compile through /api/build/compile.")
      printf '%s\n' "当前代理请求通过 /api/build/compile 编译当前项目。"
      ;;
    "Codex requested compile for the current project and will continue after callback")
      printf '%s\n' "当前代理请求编译当前项目，并将在收到回调后继续原任务。"
      ;;
    "Compile signIn after Codex edits")
      printf '%s\n' "Codex 修改后编译 signIn。"
      ;;
...
require_api_available() {
  if ! terminal_sessions_json >/dev/null 2>&1; then
    echo "webClx API unavailable at $BASE_URL; start or repair webClx before queueing this compile." >&2
    exit 1
  fi
}
...
payload=$(jq -nc \
  ...
  --argjson skip_sync "$SKIP_SYNC" \
  --argjson skip_restart "$SKIP_RESTART" \
  '{
